...
const state = {
  ...
  currentPrompt: null,
  currentFocus: { key: 'summary', label: '小科分析' },
  topicChips: [],
  summaryDirty: false,
...
async function appendContextMessage(value) {
  ...
  if (state.generationReady) {
    ...
    await showGenerationConfirmCard('如果你还有别的信息，也可以继续补充；如果没有，现在就可以直接生成分析。');
    return;
  }
...
async function showSummaryRefreshPrompt(note = '根据你刚补充的新信息，我可以把这次分析更新得更准确。') {
  ...
  await revealChoiceBlock(
    ...
    async (option) => {
      ...
      await addBotText('可以，我们继续聊。等你想更新时，再点分析里的相关主题也行。');
      $('composerInput').focus();
    },
    note,
    '可随时更新',
...
function focusResultTopic(topicKey) {
  ...
  if (topicKey === 'summary') {
    state.resultAnchor = 'summary';
    setCurrentFocus({ key: 'summary', label: '小科分析' });
    requestAnimationFrame(() => {
...
async function shareCurrentSummary() {
  ...
  state.shareUrl = url;
  const title = `挂啥科分析：${state.triageResult.layeredOutput.core.suggestDepartment}`;
  ...
  try {
    ...
    await addBotText('分析链接已经复制好了，可以直接发给家属。');
  } catch (_error) {
...
function normalizeCheckName(value = '') {
  return String(value || '')
    .replace(/\s+/g, '')
    .replace(/[（(].*?[）)]/g, '')
    .replace(/检查|检验|测定|检测|项目/g, '')
    .trim()
    .toLowerCase();
}

function pickRecommendedChecks(examAdvice = [], pricedChecks = [], fallbackChecks = []) {
  const adviceText = (examAdvice || []).join('；');
  const priced = dedupeChecksByName(pricedChecks);
  const matched = priced.filter((item) => {
    const name = String(item?.name || '').trim();
    if (!name) return false;
    const normalizedName = normalizeCheckName(name);
    return adviceText.includes(name) || (normalizedName && normalizeCheckName(adviceText).includes(normalizedName));
  });
  if (matched.length) return dedupeChecksByName(matched);
  if (fallbackChecks.length) return dedupeChecksByName(fallbackChecks);
  return priced.slice(0, 4);
}

function buildCostHtml(cost, recommendedChecks = [], options = {}) {
...
async function renderResultCards() {
  ...
  const needsCost = Boolean(triage.core.needsCost || needsInPerson || medicationAdvice.length);
  if (!needsInPerson) {
    state.showBookingPanel = false;
  } else if (!state.showBookingPanel) {
    state.showBookingPanel = true;
  }
  ...
  const baseChecksFromCost = Array.isArray(cost?.expanded?.feeItems) ? dedupeChecksByName(cost.expanded.feeItems) : [];
  const secondRoundChecks = Array.isArray(cost?.expanded?.secondRoundChecks) ? dedupeChecksByName(cost.expanded.secondRoundChecks) : [];
  const pricedCheckCatalog = dedupeChecksByName([...baseChecksFromCost, ...secondRoundChecks]);
  ...
  const order = needsInPerson
    ? ['care', 'checks', 'cost', 'booking', 'medication']
    : ['care', 'medication', 'cost'];
  const topicChips = order
    .filter((key) => sectionVisible[key])
    .map((key) => ({ key, label: SECTION_LABELS[key] }));
  state.topicChips = topicChips;
  if (!topicChips.find((chip) => chip.key === state.currentFocus?.key)) {
    state.currentFocus = { key: 'summary', label: SECTION_LABELS.summary };
  }

  const summaryHtml = [
    '<div class="summary-card">',
    '<div class="summary-top">',
    buildSummarySectionHeading('小科分析'),
  ...
  if (sectionVisible.checks) {
    essentialChecks = buildResultCard(
      '检查项目',
      [
        examAdvice.length ? buildAdviceListHtml(examAdvice) : '',
        buildCheckListHtml(recommendedChecks, recommendedChecks.length ? '推荐检查项目（含价格）' : ''),
      ].join(''),
      'strong'
    );
  ...
  const actionRow = markResultRow(addRow(
    ...
    [
      ...
      `<p>${state.sharedView ? '家属可以根据这份分析，继续陪你处理这次问题。' : imageHighRisk ? '图片提示风险偏高，建议优先线下就医。' : needsInPerson ? '需要线下就医时，可查看“推荐医院科室”。' : '先按建议处理，也可以继续补充后再更新分析。'}</p>`,
      '</div>',
  ...
  const chipHandlers = summaryRow.querySelectorAll('[data-topic-chip]');
  chipHandlers.forEach((button) => {
    button.onclick = () => {
...
async function handlePickedFile(file, label) {
  ...
  if (state.triageResult) {
    await addBotText('我把这份材料合进这次咨询里了，给你更新一下分析。');
    await directResult();
    return;
  }

  if (state.generationReady || state.awaitingContext === 'supplement') {
    await addBotText('这份材料我已经并到这次咨询里了。你还可以继续补充，或者现在直接生成分析。');
    await showGenerationConfirmCard('这份材料已经加入分析。你可以继续补充，或者现在直接生成分析。');
    return;
  }
...
async function sendRecordAsContext(record) {
  ...
  if (state.triageResult && supplementResult.refreshSummary) {
    if (supplementResult.affectsSummary) {
      ...
      if ((supplementResult.impactLevel || 'none') === 'major') {
        await showSummaryRefreshPrompt('这条历史记录可能会影响当前分析。');
        return;
      }
  ...
  if (state.generationReady) {
    await showGenerationConfirmCard('历史记录已经补进来了。你还可以继续补充，或者现在直接生成分析。');
  }
}
